...
export default function CardFilm({filme}){
    ...
    function desfavoritar(){
        setFavorito(false)    
        let favoritos = JSON.parse( localStorage.getItem("favoritos" ) ) || []
        const favoritosAtualizado = favoritos.filter(f => f.id !== filme.filme.id)
        ...
        <div className="flex flex-col items-center justify-between gap-1 w-40 m-2 relative">
            { favorito ?
                <HeartIcon 
                    ...
                    :
                    <HeartIconOutline 
                    className="h-6 w-6 text-red-600 absolute top-1 right-2 cursor-pointer text-white hover:text-red-600" 
                    onClick={favoritar}
                    />
            }
